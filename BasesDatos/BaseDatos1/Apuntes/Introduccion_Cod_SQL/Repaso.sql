...
select 52*3 uno, 60/3 divsion, 10+7 suma, 5-2 resta;

select clave_alu, nombre, ap_paterno, ap_materno, sexo from alumnos;

select clave_alu, nombre, ap_paterno, ap_materno, peso, estatura,   
sysdate(), peso * 1000 pesogramos, (estatura*100) as estaturacm,   
(peso/(estatura*estatura)) imc, concat_ws(' ', nombre, ap_paterno, ap_materno) nombre_completo
from alumnos;

select distinct ap_materno from alumnos;

select nombre, ap_paterno, ap_materno, sexo, sexo = 'm'
from alumnos
where sexo != 'm';

select ciudad, count(*) cuenta from alumnos
group by ciudad;
...
select clave_alu, nombre from alumnos where sexo = 'f'
UNION
select clave_prof, nombres from profesores where sexo = 'f';
